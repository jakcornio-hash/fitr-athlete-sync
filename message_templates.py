...
CONCERN_TYPES = frozenset({
    "recovery_flag", "performance_concern", "re_engage", "check_in_28d",
    "never_logged", "nudge_to_log",
})
...
def _signal_narrative(signals):
    ...
    for s in signals:
        ...
        elif rt in ("re_engage", "check_in_28d"):
            days = ctx.get("days", "")
...
def _build_templates():
    ...
    t[("re_engage", "challenge")] = lambda f, ctx: (
        f"Hey {f}, it's been {_days_phrase(ctx)} without a result and I've noticed. "
        f"You're not one to go quiet for no reason. What's got in the way?"
    )
    t[("re_engage", "logic")] = lambda f, ctx: (
        f"Hey {f}, there's a {_days_phrase(ctx)} gap in your log. "
        f"Without the data I can't program you properly. What's been going on?"
    )
    t[("re_engage", "warmth")] = lambda f, ctx: (
        f"Hey {f}, it's been {_days_phrase(ctx)} since your last log and I've been thinking about you. "
        f"No pressure at all. How are things going outside of training?"
    )
    t[("re_engage", "autonomy")] = lambda f, ctx: (
        f"Hey {f}, {_days_phrase(ctx)} without a result. No assumptions from me, life happens. "
        f"What's been going on, and what would actually help right now?"
    )
    t[("re_engage", "social")] = lambda f, ctx: (
        f"Hey {f}, feels like we've lost touch a bit, {_days_phrase(ctx)} since your last result. "
        f"What have you been up to?"
    )
    t[("re_engage", "belief")] = lambda f, ctx: (
        f"Hey {f}, {_days_phrase(ctx)} without a log doesn't wipe out the work, but I can't see it "
        f"without the record. What's been going on?"
    )
    t[("re_engage", "generic")] = lambda f, ctx: (
        f"Hey {f}, just checking in, it's been {_days_phrase(ctx)} since your last result. "
        f"How are you getting on?"
    )

    # ── never_logged ─────────────────────────────────────────────────────────
    # Jak's copy, verbatim, and deliberately the same for every archetype.
    # Every previous variant led with the logging ("no results from you yet",
    # "I've not got a result from you yet"). They know they haven't logged.
    # Saying it opens with their failure and invites an excuse. Asking what made
    # them sign up restarts the relationship instead of chasing the behaviour,
    # and their answer is what tells you where to go next.
    _never_logged = lambda f, ctx: (
        f"Hey {f}! It's Jak. Just curious, what originally had you sign up with us?"
    )
    for _cluster in ("challenge", "logic", "warmth", "autonomy", "social", "belief", "generic"):
        t[("never_logged", _cluster)] = _never_logged

    # ── check_in_28d ─────────────────────────────────────────────────────────
    # Jak's copy, verbatim. Split out from re_engage: the dashboard already
    # separates a 28-44 day check-in from a 45+ re-engagement, but both used to
    # send identical copy, so someone a month quiet and someone three months
    # gone got the same message. This one says "about a month" and means it.
    _check_in_28d = lambda f, ctx: (
        f"Hey {f}, I noticed it's been about a month since you last logged a score. "
        f"You are clearly committed because you decided to sign up with us, I know we "
        f"can help you, and I want to help begin to rebuild that momentum that you've "
        f"maybe lost. Did life just get completely crazy this past month, or is there "
        f"something about the programming that isn't working for you right now?"
    )
    for _cluster in ("challenge", "logic", "warmth", "autonomy", "social", "belief", "generic"):
        t[("check_in_28d", _cluster)] = _check_in_28d

    # ── celebrate ────────────────────────────────────────────────────────────
    t[("celebrate", "challenge")] = lambda f, ctx: (
        f"Hey {f}, {ctx.get('result', 'great result')}. That's what this programme's built for. "
        f"What do we go after next?"
    )
    t[("celebrate", "logic")] = lambda f, ctx: (
        f"Hey {f}, {ctx.get('result', 'great result')}. That's the number I've been watching for, "
        f"the direction's right. What do you want to build on from here?"
    )
    t[("celebrate", "warmth")] = lambda f, ctx: (
        f"Hey {f}, just saw your result, {ctx.get('result', 'really well done')}. "
        f"You've put the work in and it's showing. How did it feel?"
    )
    t[("celebrate", "autonomy")] = lambda f, ctx: (
        f"Hey {f}, loved seeing {ctx.get('result', 'that result')}. You made that happen. "
        f"What do you want to go after next?"
    )
    t[("celebrate", "social")] = lambda f, ctx: (
        f"Hey {f}, had to reach out, {ctx.get('result', 'brilliant result')}. "
        f"How did it feel? I'd love to hear your version of it."
    )
    t[("celebrate", "belief")] = lambda f, ctx: (
        f"Hey {f}, {ctx.get('result', 'great result')}. Sit with that one, you earned it. "
        f"What do you want that result to mean for you?"
    )
    t[("celebrate", "generic")] = lambda f, ctx: (
        f"Hey {f}, great result, {ctx.get('result', 'well done')}. That consistency's paying off. "
        f"What do you want to build on next?"
    )

    # ── consistency ──────────────────────────────────────────────────────────
    t[("consistency", "challenge")] = lambda f, ctx: (
        f"Hey {f}, {_weeks_phrase(ctx)} of logging on the bounce. That's discipline, and it's the "
        f"foundation everything else gets built on. Keep it going."
    )
    t[("consistency", "logic")] = lambda f, ctx: (
        f"Hey {f}, {_weeks_phrase(ctx)} straight of data on the record. Your numbers are now solid "
    ...
    t[("consistency", "social")] = lambda f, ctx: (
        f"Hey {f}, {_weeks_phrase(ctx)} straight, love to see it. "
        f"What's been keeping you so consistent?"
    )
    t[("consistency", "belief")] = lambda f, ctx: (
        f"Hey {f}, {_weeks_phrase(ctx)} logged in a row. Each one's a brick in the wall. "
    ...
    t[("performance_concern", "challenge")] = lambda f, ctx: (
        f"Hey {f}, I've been keeping an eye on your numbers and one's dipped ({ctx.get('bench', 'a benchmark')}). "
    ...
    t[("performance_concern", "logic")] = lambda f, ctx: (
        f"Hey {f}, there's a downward trend I want to flag ({ctx.get('bench', 'a benchmark')}). "
    ...
    t[("performance_concern", "warmth")] = lambda f, ctx: (
        f"Hey {f}, something I wanted to flag rather than let sit. One of your numbers has softened lately "
    ...
    t[("performance_concern", "autonomy")] = lambda f, ctx: (
        f"Hey {f}, one of your numbers has dipped ({ctx.get('bench', 'a benchmark')}) and I want your take "
    ...
    t[("performance_concern", "social")] = lambda f, ctx: (
        f"Hey {f}, one of your numbers has dipped a bit ({ctx.get('bench', 'a benchmark')}) and I'd rather "
    ...
    t[("performance_concern", "belief")] = lambda f, ctx: (
        f"Hey {f}, one of your numbers has dipped ({ctx.get('bench', 'a benchmark')}), but your capacity "
...
def _build_combined_templates():
    ...
    t[("combined", "challenge")] = lambda f, signals: (
        f"Hey {f}, a few things have come up together that are worth a direct chat. "
    ...
    t[("combined", "logic")] = lambda f, signals: (
        f"Hey {f}, a few things I want to flag together rather than in isolation. "
    ...
    t[("combined", "warmth")] = lambda f, signals: (
        f"Hey {f}, a few things have come up at once I'd rather check in about than let sit. "
    ...
    t[("combined", "autonomy")] = lambda f, signals: (
        f"Hey {f}, a few signals have come in together this week. "
    ...
    t[("combined", "social")] = lambda f, signals: (
        f"Hey {f}, a couple of things worth a proper catch-up about. "
    ...
    t[("combined", "belief")] = lambda f, signals: (
        f"Hey {f}, a few things have surfaced together. "
    ...
    t[("combined", "generic")] = lambda f, signals: (
        f"Hey {f}, a few things have come up I want to check in about. "
